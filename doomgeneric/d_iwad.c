...
// TODO(gmb)
     ...
     && valtype == REG_SZ)
    {
        ...
        result = alloc(get_heap_allocator(), len);

        if (RegQueryValueEx(key, reg_val->value, NULL, &valtype,
                            (unsigned char *) result, &len) != ERROR_SUCCESS)
        {
            dealloc(get_heap_allocator(), result);
            result = NULL;
        }
    }
...
static void CheckUninstallStrings(void)
{
    ...
    for (i=0; i<arrlen(uninstall_values); ++i)
    {
        ...
        if (unstr == NULL)
        {
            dealloc(get_heap_allocator(), val);
        }
        else
        {
...
static void CheckCollectorsEdition(void)
{
    ...
    dealloc(get_heap_allocator(), install_path);
}
...
static void CheckSteamGUSPatches(void)
{
    ...
    patch_path = alloc(get_heap_allocator(), len);
    ...
    dealloc(get_heap_allocator(), patch_path);
    dealloc(get_heap_allocator(), install_path);
}
...
char *D_FindWADByName(char *name)
{
    ...
    for (i=0; i<num_iwad_dirs; ++i)
    {
        ...
        dealloc(get_heap_allocator(), path);
    }
...
const iwad_t **D_FindAllIWADs(int mask)
{
    ...
    result = alloc(get_heap_allocator(), sizeof(iwad_t *) * (arrlen(iwads) + 1));
    result_len = 0;
