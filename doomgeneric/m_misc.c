...
boolean M_WriteFile(char *name, void *source, int length)
{
    ...
    ok = os_file_write_bytes(handle, source, length);
    fclose(handle);
                      ...
                      const char *replacement)
{
    ...
    result = alloc(get_heap_allocator(), result_len);
    if (result == NULL)
    {
...
char *M_OEMToUTF8(const char *oem)
{
    ...
    tmp = alloc(get_heap_allocator(), len * sizeof(wchar_t));
    MultiByteToWideChar(CP_OEMCP, 0, oem, len, tmp, len);
    result = alloc(get_heap_allocator(), len * 4);
    WideCharToMultiByte(CP_UTF8, 0, tmp, len, result, len * 4, NULL, NULL);
    dealloc(get_heap_allocator(), tmp);
